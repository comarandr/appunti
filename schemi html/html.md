...
Il comportamento dipende dal tipo di file (htm apertura pagina, immagini vengono visualizzate, doc e pdf vengono aperti se c’è il plugin, zip/com/exe vengono scaricati). Possibile anche mandare una mail con href=”mailto:indirizzo@mail”
...
o	shape indica la forma dell’area: rect, circle, poly
o	coords delimita superficie in pixel: per rettangolo (colonna vertice alto sx, riga vertice alto sx, colonna vertice basso dx, riga vertice basso dx) , per cerchio (centro, raggio) e poligono (tutti vertici)
o	href richiama il link associato, volendo puoi usare nohref
o	title per tooltip quando cursore si ferma

FRAME
Utili per dividere la pagine in sezioni indipendenti tra loeo, usati al posto del tag <body>. 
...
o	multipart/form-data (per upload di file sul server, permette inviare dati come sequenza di parti)
...
o	size, numero di opzioni visibili nella lista senza scrollare con barra di scorrimento
o	multiple, se posso selezionare più elementi contemporaneamente 
o	selected, indica una voce di menù selezionata
o	value, NON facoltativo, indica il valore che verrà inviato
